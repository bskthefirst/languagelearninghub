...
async function hydrateStateFromServer() {
  ...
  try {
    const response = await fetch(`/api/state?user=${encodeURIComponent(ui.activeUserId)}`);
    if (!response.ok) {
...
function queueRemoteSave(immediate = false) {
  ...
  remoteSaveTimer = setTimeout(async () => {
    try {
      const response = await fetch(`/api/state?user=${encodeURIComponent(targetUserId)}`, {
        method: "POST",
...
async function deleteUser(userId) {
  try {
    let response = await fetch(`/api/users/${encodeURIComponent(String(userId || ""))}`, {
      method: "DELETE",
    });
...
async function fetchSearchCached(query) {
  ...
  try {
    const response = await fetch(`/api/search?query=${encodeURIComponent(queryText)}`);
